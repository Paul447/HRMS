    ...
    if (data.users_clock_data?.length > 0) {
        data.users_clock_data.forEach(userData => {
            ...
            for (let i = 0; i < week1MaxRows; i++) {
                ...
                tableHtml += `
                    <tr class="bg-white border-b hover:bg-gray-50 ${isHoliday ? 'bg-green-50' : ''}">
                        ...
                        ${i === 0 ? `<td class="px-6 py-4 whitespace-nowrap text-gray-900 border-r border-gray-200" rowspan="${week1MaxRows}">
                            ...
                            <span class="text-xs text-gray-500">Punch: ${Number(userData?.week_1_total_hours || 0).toFixed(2)} hrs</span><br>
                            <span class="text-xs text-gray-500">PTO: ${Number(userData?.week_1_pto_total_hours || 0).toFixed(2)} hrs</span><br>
                            <span class="text-xs text-gray-500">Holiday: ${Number(userData?.week_1_holiday_total_hours || 0).toFixed(2)} hrs</span>
                        </td>` : ''}

                        <td class="px-6 py-4 whitespace-nowrap">
                            ${entry ? `
                                <div class="text-gray-900">
                                    ${(isPunch || isHoliday) ? formatDate(entry.clock_in_time) : formatDate(entry.start_date_time)}
                                </div>
                                <div class="text-gray-500 text-xs">
                                    ${(isPunch || isHoliday) ? formatOnlyTime(entry.clock_in_time) : formatOnlyTime(entry.start_date_time)}
                                </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            ` : `<span class="inline-flex items-center text-green-600 font-medium animate-pulse">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap border-r border-gray-200">
                            ${entry ? `<span class="font-medium text-gray-900">
                                ${Number((isPunch || isHoliday) ? entry.hours_worked : entry.total_hours || 0).toFixed(2)} hrs
                            </span>` : '<span class="text-gray-400">0.00 hrs</span>'}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            ${entry ? `<span class="font-medium ${isPunch ? 'text-gray-900' : (isHoliday ? 'text-green-700' : 'text-purple-700')}">${entry.type}</span>` : '<span class="text-gray-400">N/A</span>'}
                        </td>

                        ${i === 0 ? `<td class="px-6 py-4 whitespace-nowrap text-gray-900 border-l border-gray-200" rowspan="${week1MaxRows}">
                            <span class="font-semibold">${Number(userData?.regular_hours_week_1 || 0).toFixed(2)}</span>
                        </td>` : ''}
                        ${i === 0 ? `<td class="px-6 py-4 whitespace-nowrap text-gray-900 border-l border-gray-200 ${Number(userData?.overtime_hours_week_1 || 0) > 0 ? 'text-red-600 font-bold' : ''}" rowspan="${week1MaxRows}">
                            <span class="font-semibold">${Number(userData?.overtime_hours_week_1 || 0).toFixed(2)}</span>
                        </td>` : ''}
                ...
                tableHtml += `
                    <tr class="bg-white border-b hover:bg-gray-50 ${isHoliday ? 'bg-green-50' : ''}">
                        ${i === 0 ? `<td class="px-6 py-4 whitespace-nowrap text-gray-900 border-r border-gray-200" rowspan="${week2MaxRows}">
                            ...
                            <span class="text-xs text-gray-500">Punch: ${Number(userData?.week_2_total_hours || 0).toFixed(2)} hrs</span><br>
                            <span class="text-xs text-gray-500">PTO: ${Number(userData?.week_2_pto_total_hours || 0).toFixed(2)} hrs</span><br>
                            <span class="text-xs text-gray-500">Holiday: ${Number(userData?.week_2_holiday_total_hours || 0).toFixed(2)} hrs</span>
                        </td>` : ''}
                        ...
                        ${i === 0 ? `<td class="px-6 py-4 whitespace-nowrap text-gray-900 border-l border-gray-200" rowspan="${week2MaxRows}">
                            <span class="font-semibold">${Number(userData?.regular_hours_week_2 || 0).toFixed(2)}</span>
                        </td>` : ''}
                        ${i === 0 ? `<td class="px-6 py-4 whitespace-nowrap text-gray-900 border-l border-gray-200 ${Number(userData?.overtime_hours_week_2 || 0) > 0 ? 'text-red-600 font-bold' : ''}" rowspan="${week2MaxRows}">
                            <span class="font-semibold">${Number(userData?.overtime_hours_week_2 || 0).toFixed(2)}</span>
                        </td>` : ''}
                    </tr>
                `;
            }
