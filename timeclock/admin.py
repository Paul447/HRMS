...
class ClockAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'user',
        'formatted_clock_in_time',
        'formatted_clock_out_time',
        'hours_worked',
        'pay_period_display',
        'status_display',
    )
    ...
    fieldsets = (
        ...
        ('Calculated Information (Auto-filled)', { # Clearer title
            ...
            'description': 'These fields are automatically computed based on clock times.'
        }),
    )
        ...
        if request.method == 'GET':
            if request.user.is_authenticated:
                ...
                if active_clock:
                    messages.warning(
                        request,
                        f"You are already clocked in since {timezone.localtime(active_clock.clock_in_time).strftime('%a %m/%d/%y %I:%M %p')}. Please clock out first or edit your existing entry."
                    )
