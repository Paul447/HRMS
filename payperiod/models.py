...
class PayPeriod(models.Model):
    ...
    start_date = models.DateTimeField(verbose_name="Start Date and Time", help_text="The exact UTC date and time when this pay period begins.")
    end_date = models.DateTimeField(verbose_name="End Date and Time", help_text="The exact UTC date and time when this pay period ends. " "Typically 23:59:59 on the end day of the period.")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Created At", help_text="The date and time when this pay period was created.")
    ...
    def get_pay_period_for_date(cls, target_date_time):
        ...
        return cls.objects.filter(start_date__lte=target_date_time, end_date__gte=target_date_time).first()
    ...
    def generate_biweekly_pay_periods(cls, num_periods=35, start_from_date=None):
        ...
        if start_from_date:
            ...
            effective_start_local_dt = local_tz.localize(datetime.combine(next_start_date_component, time(0, 0, 0)))
        else:
        ...
        for i in range(num_periods):
            ...
            try:
                current_end_local_dt = local_tz.localize(datetime.combine(current_end_date_component, time(23, 59, 59)))
            except pytz.AmbiguousTimeError:
                current_end_local_dt = local_tz.localize(datetime.combine(current_end_date_component, time(23, 59, 59)), is_dst=True)
                ...
                next_day_midnight = local_tz.localize(datetime.combine(current_end_date_component + timedelta(days=1), time(0, 0, 0)))
            ...
            if cls.objects.filter(start_date__lte=end_date_utc, end_date__gte=start_date_utc).exclude(pk__in=cls.objects.filter(start_date=start_date_utc, end_date=end_date_utc)).exists():
            ...
            cls.objects.create(start_date=start_date_utc, end_date=end_date_utc)
            ...
            current_start_local_dt = local_tz.localize(datetime.combine(next_start_date_component, time(0, 0, 0)))
