...
class PayPeriodAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('generate-pay-periods/', self.admin_site.admin_view(self.generate_pay_periods_view), name='payperiod_generate_pay_periods'),
        ]
    ...
    def generate_pay_periods_view(self, request):
        ...
        if request.method == 'POST':
            ...
            created_count, initial_start_date = PayPeriod.generate_biweekly_pay_periods(
                num_periods=num_periods_to_generate
            )
            ...
            if created_count > 0:
                ...
                self.message_user(
                    request,
                    f"Successfully created {created_count} biweekly pay periods starting from {local_initial_start}.",
                    level=messages.SUCCESS
                )
            else:
                self.message_user(
                    request,
                    "No new pay periods were generated. They might already exist or an issue occurred.",
