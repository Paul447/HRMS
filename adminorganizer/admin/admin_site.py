...
class CustomAdminSite(admin.AdminSite):
    ...
    def get_app_list(self, request):
        ...
        def create_model_entry(model_class, name, app_label):
            return {"model": model_class, "name": name, "admin_url": urls_reverse(f"admin:{app_label}_{model_class._meta.model_name}_changelist", current_app=self.name), "add_url": urls_reverse(f"admin:{app_label}_{model_class._meta.model_name}_add", current_app=self.name), "object_name": model_class._meta.model_name, "perms": {"add": request.user.has_perm(f"{app_label}.add_{model_class._meta.model_name}"), "change": request.user.has_perm(f"{app_label}.change_{model_class._meta.model_name}"), "delete": request.user.has_perm(f"{app_label}.delete_{model_class._meta.model_name}"), "view": request.user.has_perm(f"{app_label}.view_{model_class._meta.model_name}")}}

        # 1. User & Organizational Management
        user_org_models = [create_model_entry(User, "Users", "auth"), create_model_entry(Group, "Groups", "auth"), create_model_entry(YearOfExperience, "Years of Experience", "yearofexperience"), create_model_entry(UserProfile, "User Profiles", "department"), create_model_entry(Department, "Departments", "department")]
        app_list.append({"name": "User & Organization", "app_label": "user_organization", "has_module_perms": True, "models": user_org_models})  # Unique label for this group

        # 2. Time Tracking
        time_tracking_models = [create_model_entry(Clock, "Clocks", "timeclock")]
        ...
        leave_balance_models = [create_model_entry(PTOBalance, "PTO Balances", "ptobalance"), create_model_entry(TimeoffRequest, "Time Off Requests", "timeoffreq"), create_model_entry(LeaveType, "Leave Types", "leavetype"), create_model_entry(DepartmentBasedLeaveType, "Department Based Leave Types", "leavetype")]
        ...
        comp_accruals_models = [create_model_entry(EmployeeType, "Employee Types", "employeetype"), create_model_entry(Pay_Frequency, "Pay Frequencies", "payfrequency"), create_model_entry(AccrualRates, "Accrual Rates", "accuralrates")]
        ...
        system_adjustments_models = [create_model_entry(BiweeklyCron, "Biweekly Cron Jobs", "biweeklycron"), create_model_entry(Holiday, "Holidays", "holiday"), create_model_entry(PayPeriod, "Pay Periods", "payperiod"), create_model_entry(AllowIpAddress, "Allowed IP Addresses", "allowipaddress"), create_model_entry(Notification, "Notifications", "notificationapp"), create_model_entry(SickLeaveProratedValue, "Sick Leave Prorated Value", "sickpolicy"), create_model_entry(MaxSickValue, "Maximum Sick Values", "sickpolicy"), create_model_entry(SickLeaveBalance, "Sick Leave Balances", "unverifiedsickleave")]
