...
class TimeoffRequest(models.Model):
    employee = models.ForeignKey("auth.User", on_delete=models.CASCADE, related_name="timeoff_requests", verbose_name="Employee")
    requested_leave_type = models.ForeignKey("leavetype.DepartmentBasedLeaveType", on_delete=models.CASCADE, related_name="timeoff_requests", verbose_name="Leave Type", help_text="Type of leave requested by the employee")
    start_date_time = models.DateTimeField(verbose_name="Start Date and Time", help_text="Start date and time of the requested leave")
    end_date_time = models.DateTimeField(verbose_name="End Date and Time", help_text="End date and time of the requested leave")
    time_off_duration = models.DecimalField(max_digits=5, decimal_places=2, default=0.0, verbose_name="Time Off Duration", help_text="Duration in hours, automatically calculated based on start and end date/time")
    employee_leave_reason = models.CharField(max_length=255, verbose_name="Leave Reason", help_text="Reason for the leave request")
    status = models.CharField(max_length=20, choices=[("pending", "Pending"), ("approved", "Approved"), ("rejected", "Rejected")], default="pending", verbose_name="Time off Status")
    reference_pay_period = models.ForeignKey("payperiod.PayPeriod", null=True, blank=True, on_delete=models.CASCADE, related_name="timeoff_requests", verbose_name="Pay Period", help_text="Pay period during which the time off is requested")
    document_proof = models.FileField(upload_to=pto_document_upload_path, blank=True, null=True, verbose_name="Document Proof", validators=[FileExtensionValidator(allowed_extensions=["pdf", "jpg", "jpeg", "png"])], help_text="Upload a document proof for applicable leave requests (only PDF, JPG, JPEG, PNG allowed)")  # Renamed from medical_document_proof  # Verbose name updated  # Help text updated
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Created At", help_text="Timestamp when the time off request was created")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Updated At", help_text="Timestamp when the time off request was last updated")
    reviewer = models.ForeignKey("auth.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="reviewed_timeoff_requests", verbose_name="Reviewed By", help_text="User who last reviewed (approved or rejected) the time off request")
    ...
    class Meta:
        ...
        ordering = ["-created_at"]
        indexes = [models.Index(fields=["employee", "status"]), models.Index(fields=["start_date_time", "end_date_time"])]
    ...
    def _split_to_across_midnight(self, local_start_time, local_original_end_time):
        ...
        while next_segment_start_local < local_original_end_time:
            ...
            if daily_segment_hours > 0:  # Only create new entries for positive durations
                ...
                new_to_entry = TimeoffRequest(employee=self.employee, requested_leave_type=self.requested_leave_type, start_date_time=next_segment_start_local.astimezone(pytz.utc), end_date_time=current_daily_segment_end_local.astimezone(pytz.utc), time_off_duration=daily_segment_hours, employee_leave_reason=self.employee_leave_reason, status=self.status, reference_pay_period=subsequent_pay_period, document_proof=self.document_proof, reviewer=None, reviewed_at=None)  # Updated
    ...
    def _split_to_at_pay_period_boundary(self, local_start_time, local_original_end_time, pay_period_boundary_local):
        ...
        if next_segment_hours > 0:  # Only create new entries for positive durations
            ...
            new_to_entry = TimeoffRequest(employee=self.employee, requested_leave_type=self.requested_leave_type, start_date_time=next_segment_start_local.astimezone(pytz.utc), end_date_time=next_segment_end_local.astimezone(pytz.utc), time_off_duration=next_segment_hours, employee_leave_reason=self.employee_leave_reason, status=self.status, reference_pay_period=subsequent_pay_period, document_proof=self.document_proof, reviewer=None, reviewed_at=None)  # Updated
            # Pass process_timeoff_logic=False to prevent further splitting on this new segment's save.
