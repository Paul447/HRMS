...
class TimeoffRequestViewSetEmployee(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = TimeoffRequest.objects.filter(employee=user).select_related("employee", "requested_leave_type", "reference_pay_period")
    ...
    def approved_and_rejected(self, request):
        ...
        approved_requests_queryset = TimeoffRequest.objects.filter(employee=user, status__iexact="approved").select_related("employee", "requested_leave_type", "reference_pay_period", "requested_leave_type__leave_type", "requested_leave_type__department")  # Corrected field name  # Optimizing query

        rejected_requests_queryset = TimeoffRequest.objects.filter(employee=user, status__iexact="rejected").select_related("employee", "requested_leave_type", "reference_pay_period", "requested_leave_type__leave_type", "requested_leave_type__department")  # Corrected field name  # Optimizing query
        ...
        if pay_period_id:
            try:
                ...
                return Response({"approved_requests": [], "rejected_requests": []})
        else:
        ...
        return Response({"approved_requests": approved_data, "rejected_requests": rejected_data})
...
class DepartmentLeaveTypeDropdownView(APIView):
    ...
    def get(self, request):
        ...
        leave_types = DepartmentBasedLeaveType.objects.filter(department__id__in=user_department_ids).select_related("department", "leave_type")

        # Return simplified JSON
        data = [{"id": lt.id, "display_name": f"{lt.leave_type.name}"} for lt in leave_types]
