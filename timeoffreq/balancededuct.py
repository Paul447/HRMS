...
def _deduct_unvsl_balance(timeoff_request_instance, hours_to_deduct):
    ...
    if balance_obj.unverified_sick_balance < hours_to_deduct_decimal:
        raise ValidationError(
            f"Insufficient unverified sick leave balance for user '{timeoff_request_instance.employee.username}'. "
            f"Required: {hours_to_deduct_decimal}, Available: {balance_obj.unverified_sick_balance}"
        )
...
def _deduct_vsl_balance(timeoff_request_instance, hours_to_deduct):
    ...
    if balance_obj.verified_sick_balance < hours_to_deduct_decimal:
        raise ValidationError(
            f"Insufficient verified sick leave balance for user '{timeoff_request_instance.employee.username}'. "
            f"Required: {hours_to_deduct_decimal}, Available: {balance_obj.verified_sick_balance}"
        )
...
def _deduct_pto_balance(timeoff_request_instance, hours_to_deduct):
    ...
    if balance_obj.pto_balance < hours_to_deduct_decimal:
        raise ValidationError(
            f"Insufficient PTO balance for user '{timeoff_request_instance.employee.username}'. "
            f"Required: {hours_to_deduct_decimal}, Available: {balance_obj.pto_balance}"
        )
