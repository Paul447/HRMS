    ...
    try:
        ...
        with get_connection(fail_silently=True) as connection:
            ...
            for recipient in recipient_list:
                msg = EmailMultiAlternatives(subject=subject, body=plain_message, from_email=settings.DEFAULT_FROM_EMAIL, to=[recipient], connection=connection)
                if html_email:
            ...
            if num_sent:
                logger.info("Email sent successfully to %s for subject: '%s'. Sent %s emails.", ", ".join(recipient_list), subject, num_sent)
            else:
                logger.warning("No emails sent to %s for subject: '%s'. Possible SMTP issue.", ", ".join(recipient_list), subject)
            return num_sent
    except (ValueError, AttributeError, TypeError) as e:
        logger.error("Error rendering template or preparing email for subject '%s': %s", subject, e, exc_info=True)
        return 0
    except Exception as e:
        logger.error("Unexpected error sending email to %s for subject '%s': %s", ", ".join(recipient_list), subject, e, exc_info=True)
        return 0
...
def get_supervisor_for_user(user):
    ...
    try:
        ...
        if department:
            supervisor_to_notify = UserProfile.objects.filter(department=department, is_manager=True).first()

            if supervisor_to_notify and supervisor_to_notify.user:
                return supervisor_to_notify, supervisor_to_notify.user.email
            else:
                logger.warning(f"No manager found in department '{department.name}' " f"for user {user.username} (ID: {user.id}).")
        else:
            logger.warning(f"User {user.username} (ID: {user.id}) has no department. " f"Cannot find department manager.")
    except UserProfile.DoesNotExist:
        logger.error(f"UserProfile not found for user {user.username} (ID: {user.id}).")
    except Exception as e:
