        ...
        if instance and instance.pk:
            if instance.status in ['approved', 'rejected'] and status != instance.status:
                ...
                raise forms.ValidationError(
                    "Cannot change the status of a request that is already '%s'." % instance.status
                )
        return status
...
class TimeoffreqAdmin(ModelAdmin):
    ...
    exclude = (
        'time_off_duration',
        'status', # Keep status here if you want it editable only via the form, not list_editable
        'reference_pay_period',
        'reviewer',
        'reviewed_at',
    )
