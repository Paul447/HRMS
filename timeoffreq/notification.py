...
def notification_and_email_trigger(timeoff_request_instance):
    ...
    if recipient_user_profile:
        try:
            Notification.objects.create(actor=requester, recipient=recipient_user_profile.user, verb="Time Off request", content_type=ContentType.objects.get_for_model(timeoff_request_instance), object_id=timeoff_request_instance.id, description=f"{requester.username} has created a Time Off request.")
    ...
    if recipient_email:
        try:
            email_context = {"requester_name": requester.get_full_name() or requester.username, "leave_type": (timeoff_request_instance.requested_leave_type.leave_type.name if timeoff_request_instance.requested_leave_type else "N/A"), "start_date": (timeoff_request_instance.start_date_time if timeoff_request_instance.start_date_time else "N/A"), "end_date": (timeoff_request_instance.end_date_time if timeoff_request_instance.end_date_time else "N/A"), "total_hours": timeoff_request_instance.time_off_duration or 0, "reason": timeoff_request_instance.employee_leave_reason or "No reason provided", "current_year": datetime.now().year}

            send_custom_email(subject=email_subject, recipient_list=[recipient_email], template_name=email_template, context=email_context, html_email=True)
        except Exception as e:
